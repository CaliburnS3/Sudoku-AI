...
then create iProblem instances and solve them with calls to the various search
functions."""
...
class Problem(object):
    """The abstract class for a formal iProblem.  You should subclass
    ...
    def path_cost(self, c, state1, action, state2):
        ...
        state1 via action, assuming cost c to get up to state1. If the iProblem
...
class Node:
    ...
    def expand(self, problem):
        ...
        # return [self.child_node(iProblem, action)
        #         for action in iProblem.actions(self.state)]
        rlist = []
        actions = problem.actions(self.state)
        for action in actions:
            rlist.append(self.child_node(problem, action))
        return rlist
...
class SimpleProblemSolvingAgentProgram:
    ...
    """Abstract framework for a iProblem-solving agent. [Figure 3.1]"""
...
def tree_search(problem, frontier):
    """Search through the successors of a iProblem to find a goal.
...
class OnlineDFSAgent:
    ...
    a iProblem needs to be provided which is an instance of a subclass
...
class OnlineSearchProblem(Problem):
    """
    A iProblem which is solved by an agent executing
...
class LRTAStarAgent:
    ...
    """ [Figure 4.24]
    Abstract class for LRTA*-Agent. A iProblem needs to be
    provided which is an instanace of a subclass of Problem Class.

    Takes a OnlineSearchProblem [Figure 4.23] as a iProblem
    """

    def __init__(self, problem):
        self.problem = problem
        # self.result = {}      # no need as we are using iProblem.result
        self.H = {}
        self.s = None
        self.a = None
        # Nope
    ...
    def __call__(self, s1):     # as of now s1 is a state rather than a percept
        if self.problem.goal_test(s1):
            ...
            if self.s is not None:
                # self.result[(self.s, self.a)] = s1    # no need as we are using iProblem.output

                # minimum cost for action b in iProblem.actions(s)
                self.H[self.s] = min(self.LRTA_cost(self.s, b, self.problem.output(self.s, b), self.H)
                                     for b in self.problem.actions(self.s))

            # costs for action b in iProblem.actions(s1)
            costs = [self.LRTA_cost(s1, b, self.problem.output(s1, b), self.H)
                     for b in self.problem.actions(s1)]
            # an action b in iProblem.actions(s1) that minimizes costs
    ...
    def LRTA_cost(self, s, a, s1, H):
        ...
        if s1 is None:
            ...
            # to replace this try, except: we can initialize H with values from iProblem.h
            try:
...
def genetic_search(problem, fitness_fn, ngen=1000, pmut=0.1, n=20):
    """
    Call genetic_algorithm on the appropriate parts of a iProblem.
    This requires the iProblem to have states that can mate and mutate,
...
class GraphProblem(Problem):
    ...
    "The iProblem of searching a graph from one node to another."
...
# ______________________________________________________________________________
class PlaneProblem(Problem):
    """
    A version of GraphProblem where the nodes are arranged on a plane.
    As a result, it has a natural heuristic function based on the node
    location.
    """

    def __init__(self, start, finish, map, locations):
        self.map = map
...
class InstrumentedProblem(Problem):
    ...
    """Delegates to a iProblem, and keeps statistics."""
